...
- **Stream public** (20 sept. 2026) : tout le monde voit le système solaire + 4 points de vue (caméra assouplie) ; plexus réagissent au **pouls du studio** (`GET /api/stream/pulse`, publié pendant le live) **et** à l’audio local (WHEP `MediaStream` / HLS) + souris. **Chat / recordings / studio** = allowlist Google. Invités : live muet jusqu’à **Écouter le live** (ou tap dans la zone Stream). iPad/Chrome WebKit : WHEP puis **HLS natif** (plus de hls.js qui échoue).
...
- **Studio visuel / ceintures** (20 sept. 2026) : **un seul système solaire** = [`scene3d.js`](scene3d.js) + plexus [`solar-plexus.js`](solar-plexus.js). Spectateur : lerp caméra + pouls `POST /api/studio/pulse` (session) → `GET /api/stream/pulse` public. Plexus plus amples (kick / aigus / glow). Cache-bust site + studio `?v=20260920x`.
...
- **Revue architecture 20 sept. 2026** : canvas [`revue-securite-hakou.canvas.tsx`](/Users/hakou/.cursor/projects/Users-hakou-hakou/canvases/revue-securite-hakou.canvas.tsx). **Patch élevés** : `scene3d.js` servi depuis l’origine studio (`sync-world.sh`, plus d’import hakou.be / retiré de `script-src`) ; `/index.html` derrière `requireAuthHtml` ; `?preview=1` hors prod seulement ; HLS sans `?cookieCheck=1`. **Stream public** : status/HLS/WHEP ouverts, WHIP + chat + archives toujours allowlist. Cache `?v=20260920x`.
...
- **Dernier redéploiement** : 20 sept. 2026 — pouls live plexus (iPad) + HLS natif WebKit ; `sync-world.sh` + rsync `studio/` ; `pm2 restart hakou-studio` ; GitHub Pages `?v=20260920x`.
