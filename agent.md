...
- **Scène** (`scene3d.js`) : groupe `introGate` — logo SVG vectoriel (stable, **sans bounce**) + voile noir opaque derrière le logo + 3 nébuleuses. Au repos : fond `#000` + voile opacity 1. Pendant `startIntroGateZoom` (~3,4 s) : le voile fond vers transparent (révèle l’univers), logo fondu doux (sans explosion d’échelle), nébuleuses s’écartent, caméra vers `sectionCameras[0]`.
