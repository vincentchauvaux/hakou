...
- **Studio visuel / ceintures** (20 sept. 2026) : visuel = plexus (jamais l’écran). Son = **partage d’écran + audio**. Plexus spectateurs = pouls `GET /api/stream/pulse` (CORS hakou.be/www, ignore pouls à zéro). 3 mouvements depuis la Terre : **bass** onde radiale, **mid** swirl orbital, **high** scintillement. Pills Stream spectateurs : largeur = contenu (plus de grille 4×1fr). Cache `?v=20260920ad`.
...
- **Revue architecture 20 sept. 2026** : canvas [`revue-securite-hakou.canvas.tsx`](/Users/hakou/.cursor/projects/Users-hakou-hakou/canvases/revue-securite-hakou.canvas.tsx). **Patch élevés** : `scene3d.js` servi depuis l’origine studio (`sync-world.sh`, plus d’import hakou.be / retiré de `script-src`) ; `/index.html` derrière `requireAuthHtml` ; `?preview=1` hors prod seulement ; HLS sans `?cookieCheck=1`. **Stream public** : status/HLS/WHEP ouverts, WHIP + chat + archives toujours allowlist. Cache `?v=20260920ad`.
