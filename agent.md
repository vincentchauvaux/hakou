...
- **Modèle spatial** : section **0 = Intro / Neptune (loin)**, section **8 = Contact / Mercure (proche Soleil)**. Avancer = index++ = vers le Soleil. **Radio** = §2 (Pluton) entre Son et Video. **Étapes intermédiaires intérieures** : §6 **Sites** (orbite 3D : Vénus), §7 **Plugin** (orbite 3D : Terre décorative, avant Contact).
...
- **Entrées inversées pour le réalisme** :
  ...
  - **Exception scroll interne avec débordement** : tant que le panel peut encore scroller dans le sens du geste (`panelCanScrollBy`), la molette / clavier laissent (ou appliquent) le scroll contenu — pas d’inversion globale. Au bord seulement : continuer vers le bas → section suivante ; vers le haut → précédente.
...
- **Scroll interne panel (`≤680px` ou laptop `min-width: 681px` + `max-height: 820px`)** : panel actif `.panel.is-active` avec `overflow-y: auto` (**scrollbar masquée** : `scrollbar-width: none` / `::-webkit-scrollbar { display: none }` — défilement trackpad/molette inchangé) entre gutters chrome (`styles.css` : `--mobile-chrome-*` mobile ; `--compact-chrome-*` laptop). `navigation.js` : `PANEL_INTERNAL_SCROLL_MQ` (alias `MOBILE_PANEL_SCROLL_MQ`). **Priorité contenu** : si `panelCanScrollBy(panel, deltaY)`, molette ne touche pas `feedGate` (scroll natif) ; clavier applique `applyPanelScrollDelta`. Seulement au bord (plus de scroll possible dans ce sens) : bas→section suivante (dir +1), haut→précédente (dir −1) — puis charge bord / `feedGate`. Sans débordement : modèle spatial inchangé (molette haut → suivant). Tampon bord : `getPanelScrollEdgeBuffer()` = `min(80px, 12vh)` ; sur panels à faible défilement, `getEffectivePanelScrollEdgeBuffer(panel)` = `min(tampon, floor(maxScroll/2))` pour que le « bas » reste atteignable (Visuel galerie conserve un tampon utile tant que `maxScroll` est grand). **Petit débordement** (`maxScroll ≤ 2 × tampon`) : `panelBottomAllowsSectionGate` exige `panelAtScrollEnd` (scroll natif au max) avant section suivante — évite le zap quand les CTA bas de carte (Sites) ne sont pas encore visibles. Stall touch : idem, uniquement si `panelAtScrollEnd`. Gate vers section suivante aussi si `panelAtScrollEnd` (scroll natif au max) ou après **4** gestes touch vers le bas sans progression de `scrollTop` près du bas (`MOBILE_TOUCH_SCROLL_STALL_MAX`, embeds type SoundCloud). **Charge bord mobile** (`needsMobileEdgeCharge` / `accumulateMobileEdgeCharge`) : si le panel a un débordement vertical et que `canFeedSectionGate(dir)` est vrai, chaque geste « vers la section » (molette, touch, clavier) remplit d’abord une charge séparée — wheel **220**, touch **108** — avant un pulse `feedGate` de **~59** (`MOBILE_EDGE_GATE_PULSE` = 42 % de 140) ; il faut environ **2–3** charges complètes + pulses pour `gateProgress ≥ 1`. Reset charge si changement de direction, sortie du bord (`canFeedSectionGate` faux), `resetGate` / `touchstart`, ou reprise du scroll contenu. Milieu de panel : scroll natif inchangé ; panel sans débordement : pas de charge bord (comportement direct). Détection débordement : 2 px (`PANEL_SCROLL_OVERFLOW_EPS`). Helpers `canFeedSectionGate`, `panelHasVerticalOverflow`, `getPanelByZone`, `resetPanelScrollTop` dans `navigation.js`. **`scrollTop`** : au départ du glide mobile, **uniquement** le panel **destination** (encore invisible en crossfade) ; le panel **sortant** garde sa position jusqu’à la fin du glide, puis `resetGlideDepartingPanelScroll()` ; panel actif aussi `resetActivePanelScroll()` en fin de glide (évite le saut visuel « depuis le haut » mid-transition). Desktop : tous les panels à 0 au `goToSection`. **Embeds iframe (Son / Radio / Visuel)** : iframes cross-origin ne propagent pas le touch au panel — `initEmbedPanelScroll` injecte `.embed-touch-layer` (z-index 2) sur `.soundcloud-embed`, `.radio-player__frame` (`position: relative`) et `.instagram-embed-panel__scroll` quand `PANEL_INTERNAL_SCROLL_MQ` ; swipe vertical dominant → scroll synthétique du panel actif (`preventDefault` + `stopPropagation` si scroll possible) ; au bord, événement laissé au `onTouchMove` global (gating section) + stall. Tap court (< **12** px, < **350** ms) → `pointer-events: none` temporaire + `click` synthétique sur l’iframe (play SoundCloud / YouTube Radio / interaction IG si le widget l’accepte). `MutationObserver` sur `#radio-player-frame` pour resync après inject HLS / YouTube. Couche retirée hors MQ. **Son / Radio / Video / Visuel / Plugin** : `padding-bottom` mobile = chrome bas + tampon (`min(80px, 12vh)` ; laptop compact `min(48px, 8vh)`). **Sites (`#venus`)** : tampon renforcé mobile `min(160px, 24vh)` + `scroll-padding-bottom` aligné ; laptop `min(88px, 14vh)` — CTA Hirakana au-dessus de la nav. Pendant glide adjacent / long jump : `overflow-y: hidden` sur le panel actif. Desktop (hauteur > 820px) inchangé.
...
- **Assets** : [`assets/logo-hakou.svg`](assets/logo-hakou.svg) chargé via **`SVGLoader`** → formes `ShapeGeometry` (vrai vectoriel 3D, pas une texture bitmap) ; PNG `logo-hakou.png` conservé en secours. Anciennes nébuleuses `assets/nebula/*.png` non utilisées (retirées de l’intro).
