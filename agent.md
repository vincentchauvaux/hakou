...
- **Échelle solaire** (`#solar-scale`, desktop gauche ~18px) : rail discret (`.solar-scale-rail`, neutre `rgba(255,255,255,~0.06)`, **sans** remplissage vertical / gradient central) + **8 graduations** fines (`.solar-scale-tick`, `data-stop` 0–7), sans libellés. Rendu **identique** au repos pendant glide / zoom / changement de section : pas de fond sur `#solar-scale`, pas de surcharge `body[data-theme="light"]` sur rail/ticks (seul `syncTheme` continue pour le menu). Marqueur orange **8×8 px**. Intro (0) en bas, Contact (7) en haut — le Soleil n’est **pas** le pôle visuel UI (reste implicite au-delà de Mercure / horizon 3D). `updateSolarScale(displaySection)` : `--scale-progress` sur `#solar-scale-marker` uniquement (`1 - section/(N-1)`). Gutter panels gauche : `--solar-scale-gutter`.
...
| `styles.css` | Thèmes panels (dont `theme-mercury` Contact), crossfade, `#solar-scale` 8 ticks, menu latéral (desktop vertical ; mobile barre bas + échelle haut + masques chrome ; **laptop compact** masque haut + scroll panel), composition `.panel-lead` / grilles, **liens contenu** (`--panel-link` : `#fff` dark/mid/mercury ; texte panel sur `theme-light` Son — pas les `.panel-btn` ni ↗ grille), **CTA RPG CR** (`.panel-btn--primary` accent fond/bordure ; `--secondary` texte panel), cube CSS 3D preview |
