...
- **Menu latéral** (`index.html`) : ordre DOM **Contact → … → Intro** (desktop : colonne droite, proche Soleil en haut). Les `data-zone-link` restent 0–5 alignés sur les sections. **`syncUI` / `syncNavLinks`** : état actif via `Number(link.dataset.zoneLink)`, **pas** l’index DOM ; **un seul** lien actif à la fois (y compris en transit). Panels : `getPanelZone(panel)` via `data-zone`. Entrée **Visuel** (`data-zone-link="3"`) entre Video et 3D.
- **Mobile (`≤680px`)** : échelle solaire **horizontale en haut** (Neptune à gauche, Soleil à droite ; marqueur `left: calc((1 - var(--scale-progress)) * 100%)`). Menu **barre pleine largeur en bas** : pills égaux (`flex: 1`), ordre visuel **Intro → … → Contact** via `order` CSS sur `data-zone-link`. Gutter panels : `--mobile-solar-top` / `--mobile-chrome-top` (pilule + safe-area) / `--mobile-chrome-bottom`. **Chrome viewport** : `body::before` / `body::after` (fixe, `z-index: 2`) masquent le contenu de `#overlay` sous l’échelle et la nav pendant glide + scroll panel ; `#solar-scale` (`z-index: 3`, fond verre) et `.side-nav` (`z-index: 4`) au-dessus. `--mobile-chrome-top` = `calc(var(--mobile-solar-top) + 32px)`.
- **Échelle solaire** (`#solar-scale`, desktop gauche ~18px) : rail vertical + **6 graduations** (`.solar-scale-tick`, `data-stop` 0–5), sans libellés texte. Intro (0) en bas, Contact (5) en haut. Marqueur via `updateSolarScale(displaySection)` : `progress = 1 - section/(sectionCount-1)` (scroll ↑ vers le Soleil = marqueur monte). `scaleSectionMax` dérivé des panels, variable CSS `--scale-progress` sur `#solar-scale-marker`. Gutter panels gauche : `--solar-scale-gutter`.
...
| `styles.css` | Thèmes panels, crossfade, `#solar-scale`, menu latéral (desktop vertical ; mobile barre bas + échelle haut + masques chrome), cube CSS 3D preview |
