...
- **Studio visuel / ceintures** (26 sept. 2026) : intro = traversée d’œil, regard **devant**. Glides = ellipse Soleil. **OVH Hub** = carte Sites **allowlist seulement**. Son live = profil accepté **ou** code ; plexus visibles pour tous. Capture Mac : **Tout l’écran** (pas fenêtre Rekordbox). Cache `?v=20260926d`.
...
- **Revue architecture 20 sept. 2026** : canvas [`revue-securite-hakou.canvas.tsx`](/Users/hakou/.cursor/projects/Users-hakou-hakou/canvases/revue-securite-hakou.canvas.tsx). **Patch élevés** : `scene3d.js` servi depuis l’origine studio (`sync-world.sh`, plus d’import hakou.be / retiré de `script-src`) ; `/index.html` derrière `requireAuthHtml` ; `?preview=1` hors prod seulement ; HLS sans `?cookieCheck=1`. **Stream** : status ouvert (sans `hlsUrl` sans cookie) ; HLS/WHEP + chat via code/session + **Get in**. Cache `?v=20260920ak`.
...
- **Stream** : mix privé. Son = allowlist **ou** code (`data-stream-listen`). Chat = **Get in**. Invités sans code = lieux + plexus. **OVH Hub** allowlist. Cache `?v=20260926d`.
...
- **Convergence héro** (`GLIDE_HERO_BLEND_START` **0,94**) : derniers 6 % — position glide → cadrage héro destination (`computeSectionCamera`). `sampleRectilinearTransfer` force `p1` à `t=1`.

- **Glide** : **ancre = Soleil**. P0/P1 = positions caméra héro. Chemin = slerp d’angle + lerp de rayon (`rectilinearPointRaw`) + léger bulge de rayon au milieu (`GLIDE_ELLIPSE_RADIUS_BULGE` 0,07) + lift Y `JOURNEY_ARC`. **Pas** de Bézier mixé (ça combattait l’ellipse et fermait trop). **+1 section** = rayon décroissant ; **−1** = croissant.
...
- Regard **repos** = cadrage héro (planète + horizon). **Transit** : lookAt = **Soleil** (ancre) ; blend héro seulement aux 20 % de départ / arrivée — plus de lerp lookAt monde planète→planète.

- Intro punch : `dir` = lookAt→caméra (extérieur) ; traversée = **−dir** ; après l’œil le regard reste **devant** (`lookEnd` / Soleil), pas sur l’œil.
...
| **Sites** (`#venus`, `data-zone="6"`) | [OVH Hub](https://vps-e09ed6db.vps.ovh.net/ovh-hub/) (allowlist) + [StreamTV](https://vps-e09ed6db.vps.ovh.net/) + [RPG CR](https://vps-e09ed6db.vps.ovh.net/rpg-cr/) + [Hirakana](https://vps-e09ed6db.vps.ovh.net/hirakana/) + [Canopée](https://xn--canope-fva.be/) + [Mimom](https://mimom.be/) + [LM Studio](https://lmstudio.ai/download) | Nav **Sites**. **OVH Hub** : `.site-card--crew`, visible seulement si `data-stream-auth="ok"` (Vincent / Anaïs). Autres cartes publiques. Masonry + logos `assets/sites/`. Orbite 3D : Vénus. |
...
- **Scène** (`scene3d.js`) : groupe `introGate` — logo SVG vectoriel (stable, **sans bounce**). Au repos : fond `#000` + **univers masqué**, cadrage **logo entier**. Clic → **traversée de l’œil droit** (~5,2 s, `punch` 0,7) : lookAt glisse vers le trou, caméra passe dedans **sur −dir**, logo fade **avant** le plan (`t` 0,38–0,58) + `FrontSide`. **Après l’œil** : regard **devant** (limbe Pluton / Soleil), même axe, sans bosse latérale. **Pas** de scale/rotation inversée.
...
- **Live studio (Étape 3)** :
  ...
  - Studio (auth) : `GET /api/studio/ingest` → WHIP + canvas plexus. **Audio Mac** : `getDisplayMedia` **Tout l’écran** + `systemAudio` ; la piste vidéo reste **allumée** (la couper coupait le son). Fenêtre Rekordbox = souvent **sans audio**. Spectateurs : allowlist ou code + **Écouter le live**. Rec VPS séparé. Destination Hakou / YouTube / Twitch.
