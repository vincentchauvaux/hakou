...
- **Studio visuel / ceintures** (20 sept. 2026) : visuel = plexus (jamais l’écran). Son HLS **sur la vidéo** (plus de `createMediaElementSource` qui volait le haut-parleur). Plexus = pouls studio. Pills Stream = largeur du texte. Cache `?v=20260920ae`.
...
- **Revue architecture 20 sept. 2026** : canvas [`revue-securite-hakou.canvas.tsx`](/Users/hakou/.cursor/projects/Users-hakou-hakou/canvases/revue-securite-hakou.canvas.tsx). **Patch élevés** : `scene3d.js` servi depuis l’origine studio (`sync-world.sh`, plus d’import hakou.be / retiré de `script-src`) ; `/index.html` derrière `requireAuthHtml` ; `?preview=1` hors prod seulement ; HLS sans `?cookieCheck=1`. **Stream public** : status/HLS/WHEP ouverts, WHIP + chat + archives toujours allowlist. Cache `?v=20260920ae`.
