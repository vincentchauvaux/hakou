...
- **Orbit manuelle au repos** : hors mode focus, le canvas reste en `pointer-events: none` (nav / panels prioritaires). **Mode observation planète** (`#planet-focus`, à droite d’Intro) : au repos (`!isAnimating`, intro done, WebGL OK), clic → `body[data-planet-focus="on"]` — hub (overlay textes, side-nav, échelle solaire, Intro) fondu ~0,55 s ; canvas `pointer-events: auto` ; **orbite libre autour du centre planète** (pas le lookAt horizon héro) + lookAt centre + **molette zoom** ; angle orbital de la planète **figé** pendant le mode ; navigation molette/clavier/touch/menu verrouillée ; Escape ou reclic (icône × « Retour ») → hub réapparaît en smooth + caméra lerp vers cadrage héro (`clearSectionUserOrbit`). Exports `setPlanetFocusMode` / `isPlanetFocusMode` (`scene3d.js`) ; `setPlanetFocus` / `isPlanetFocusActive` (`navigation.js`).

- **Orbit manuelle (détail technique)** : clic-glisser sur `#three-canvas` **uniquement** en mode focus (`planetFocusMode`). Rotation azimut / élévation autour du **`lookAt` héro** à rayon variable (zoom). Offsets (`sectionUserOrbit`) réinitialisés au début de chaque glide et à la sortie du focus. `navigation.js` ignore le touch vertical si `isRestOrbitDragging()`. Curseur `grab` / `grabbing`.
