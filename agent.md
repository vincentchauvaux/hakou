...
- **Studio visuel / ceintures** (20 sept. 2026) : visuel = plexus (jamais l’écran). Son = **partage écran audio-only** (getDisplayMedia, piste vidéo jetée) ou **USB FLX4** (carte son, pas micro Mac). Vu-mètre avant Direct. Cache `?v=20260920z`.
...
- **Revue architecture 20 sept. 2026** : canvas [`revue-securite-hakou.canvas.tsx`](/Users/hakou/.cursor/projects/Users-hakou-hakou/canvases/revue-securite-hakou.canvas.tsx). **Patch élevés** : `scene3d.js` servi depuis l’origine studio (`sync-world.sh`, plus d’import hakou.be / retiré de `script-src`) ; `/index.html` derrière `requireAuthHtml` ; `?preview=1` hors prod seulement ; HLS sans `?cookieCheck=1`. **Stream public** : status/HLS/WHEP ouverts, WHIP + chat + archives toujours allowlist. Cache `?v=20260920z`.
...
- **Dernier redéploiement** : 20 sept. 2026 — son du mix sans image (écran audio / FLX4), pas un micro ; cache `?v=20260920z`.
