...
- **Scène** (`scene3d.js`) : groupe `introGate` — logo SVG vectoriel (stable, **sans bounce**). Au repos : fond `#000` + **univers masqué** (pas de plan-voile avec bords visibles). Pendant `startIntroGateZoom` (~3,4 s) : univers réaffiché, brouillard noir qui se dissipe, nébuleuses apparaissent puis s’écartent, logo fondu doux, caméra vers `sectionCameras[0]`.
