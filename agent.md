...
- **Stream public** (20 sept. 2026) : tout le monde voit le système solaire + 4 points de vue (caméra assouplie) ; plexus réagissent au HLS public **et à la souris** (puits d’attraction + léger peek caméra, comme un doigt). **Chat / recordings / studio** = allowlist Google (`ALLOWED_EMAILS` VPS — Vincent & Anaïs). Bouton bas-gauche `#chrome-login` « Se connecter » (GIS) → studio ; plus de `#intro-replay` / `#stream-lock`. Invités : `body[data-stream-auth="guest"]` masque `.stream-crew`, HLS muet + `#stream-listen`.
...
- **Studio visuel / ceintures** (20 sept. 2026) : **un seul système solaire** = [`scene3d.js`](scene3d.js) + plexus [`solar-plexus.js`](solar-plexus.js) (4 ceintures toujours visibles). **Spectateur Stream** (§2) : 4 scènes (Principale / L4 / Kuiper / Saturne) via [`stream-scenes.js`](stream-scenes.js) — caméra **et Voir** suivent le spot (L4 → Jupiter, pas Uranus), lerp pos/lookAt (pas de snap). **Studio rec/live** : audio → `setAudioVibe` / `tickPlexus`, tous les plexus dansent. **Son à caster** : canvas 3D `pointer-events: none` (le dock reste cliquable) ; bouton **Lister les micros** (permission + labels BlackHole/Rekordbox) ; puces d’entrées ; iPhone/Continuity filtrés. Cache-bust site + studio `?v=20260920w`. **Login Google Cursor** : tester dans Chrome. Aperçu studio : `?preview=1` **hors prod seulement**.
...
- Config Twitch (VPS `/opt/hakou-studio/.env`) :
  ...
  - Redirect : `https://studio.hakou.be/api/studio/twitch/callback`
...
- Note : HLS `/hakou-live/hls/` et WHEP `/hakou-live/whip/…/whep` sont **publics** ; WHIP publish `/hakou-live/whip/hakou/whip` reste `auth_request` + cookie `hakou_media`. Pas de jetons `?session=` côté client.
...
- **Revue architecture 20 sept. 2026** : canvas [`revue-securite-hakou.canvas.tsx`](/Users/hakou/.cursor/projects/Users-hakou-hakou/canvases/revue-securite-hakou.canvas.tsx). **Patch élevés** : `scene3d.js` servi depuis l’origine studio (`sync-world.sh`, plus d’import hakou.be / retiré de `script-src`) ; `/index.html` derrière `requireAuthHtml` ; `?preview=1` hors prod seulement ; HLS sans `?cookieCheck=1`. **Stream public** : status/HLS/WHEP ouverts, WHIP + chat + archives toujours allowlist. Cache `?v=20260920w`.
...
- **Dernier redéploiement** : 20 sept. 2026 — patch sécu élevés (scene3d même origine, gate `/index.html`, HLS sans `?session=`) ; `sync-world.sh` + rsync `studio/` ; `pm2 restart hakou-studio` ; GitHub Pages `radio.js` / `scene3d.js` ; cache `?v=20260920q`.
...
| `studio/` | Service Node VPS : auth allowlist + page studio (même `scene3d.js`) ; picker **Son à caster** (micros / onglet) |
...
| **Sites** (`#venus`, `data-zone="6"`) | [StreamTV](https://vps-e09ed6db.vps.ovh.net/) + [RPG CR](https://vps-e09ed6db.vps.ovh.net/rpg-cr/) + [Hirakana](https://vps-e09ed6db.vps.ovh.net/hirakana/) + [Canopée](https://xn--canope-fva.be/) + [Mimom](https://mimom.be/) + [LM Studio](https://lmstudio.ai/download) | Nav libellé **Sites**. Galerie **masonry** (`.sites-grid` : `columns: 2` desktop / laptop, `1` mobile ; `break-inside: avoid` sur `.site-card`). Cartes avec **favicon** (`.site-card__logo` 40 px, assets `assets/sites/`) : StreamTV (accueil VPS, SVG « S »), Hirakana (`apple-touch`), RPG CR (d20 — pas de favicon source), Canopée (`.site-card--feature`, kicker « Pour ma maman », `apple-icon`), **Mimom** (suivi bébé, `https://mimom.be/`, kicker « Famille », logo app). Orbite 3D : Vénus. |
...
- **Scène** (`scene3d.js`) : groupe `introGate` — logo SVG vectoriel (stable, **sans bounce**). Au repos : fond `#000` + **univers masqué**. Clic logo → **traversée de l’œil** (~5,2 s) : caméra dollie dans le trou du masque (depthTest), univers visible à travers, puis cadrage §0 live. Fond **reste noir** ; logo opaque jusqu’après le passage ; **pas de nébuleuses** ; à la fin `finishIntroZoomToLive` aligne caméra / FOV / `introSnapFrames` sans reset brutal.
...
- **Auth Google (Étape 2)** :
  ...
  - Nginx : vhost **studio.hakou.be** ([`studio/deploy/nginx-studio.hakou.be.conf.example`](studio/deploy/nginx-studio.hakou.be.conf.example)) + snippet `/etc/nginx/snippets/hakou-studio.conf` sur le hostname OVH (redirige `/hakou-studio/` → studio.hakou.be). Cookie session Path=`/` sur le sous-domaine, Path=`/hakou-studio` sur l’ancien host.
  - Après login OK : zoom intro (§0) puis **saut long** jusqu’au Soleil (§8 Contact, même glide que le menu) ; overlay masqué (`data-studio-arrive`) ; à l’arrivée **redirect** `https://studio.hakou.be/`. Sans session → 401.
...
- **Live studio (Étape 3)** :
  ...
  - Nginx : `/hakou-live/whip/hakou/whip` → WHIP **auth** ; `/hakou-live/whip/` (WHEP) et `/hakou-live/hls/` → **public** ([`studio/deploy/nginx-hakou-live.conf.example`](studio/deploy/nginx-hakou-live.conf.example)). **Pas** de `?cookieCheck=1`. Client [`radio.js`](radio.js) : `xhr.withCredentials = true`. Safari / iOS : WHEP.
  ...
  - Spectateurs : [`radio.js`](radio.js) public si `studioLive` / Twitch / YouTube. Chat / recordings seulement si session. Hors antenne : **logo Hakou**. Priorité studio > Twitch live > YouTube live.
