...
- **Orbit manuelle au repos** : hors mode focus, le canvas reste en `pointer-events: none` (nav / panels prioritaires). **Mode observation planète** (`#planet-focus`, à droite d’Intro) : au repos (`!isAnimating`, intro done, WebGL OK), clic → `body[data-planet-focus="on"]` — hub (overlay textes, side-nav, échelle solaire, Intro) fondu ~0,55 s ; canvas `pointer-events: auto` ; **orbite libre autour du centre planète** + lookAt centre + **molette / pinch zoom** ; planètes **continuent leur orbite Kepler** (caméra suit le mesh) ; **rotation propre** continue sauf **pendant grab/pinch** (gel sans saut, reprise au relâchement) ; drag caméra avec **inertie** (freinage expo → repos) ; navigation molette/clavier/touch/menu verrouillée ; Escape ou reclic (icône × « Retour ») → hub réapparaît en smooth + caméra lerp vers cadrage héro (`clearSectionUserOrbit`). Exports `setPlanetFocusMode` / `isPlanetFocusMode` (`scene3d.js`) ; `setPlanetFocus` / `isPlanetFocusActive` (`navigation.js`).

- **Orbit manuelle (détail technique)** : clic-glisser sur `#three-canvas` **uniquement** en mode focus (`planetFocusMode`). Rotation azimut / élévation autour du **centre planète** à rayon variable (zoom). **Pinch** 2 doigts (mobile) change le rayon. Relâchement : `azVel`/`elVel` amortis (`FOCUS_ORBIT_INERTIA_DAMP`) + reprise spin (`sectionSpinClockOffset`). Offsets (`sectionUserOrbit`) réinitialisés au début de chaque glide et à la sortie du focus. `navigation.js` ignore le touch vertical si `isRestOrbitDragging()`. Curseur `grab` / `grabbing`. Spins : périodes sidérales compressées (`spinSpeedFromPeriodHours`) × `PLANET_SPIN_MUL` ; Uranus / Vénus rétrogrades ; nuages Vénus ~4 j rétrogrades.
...
- **UI** (`#intro-gate`) : hit-area `#intro-enter` centrée, hint « Cliquer le logo pour entrer », bouton `#intro-login` haut-droite. **`.chrome-actions`** bas-gauche (visible si `data-intro="done"`) : `#intro-replay` → `replayIntroGate()` ; `#planet-focus` → mode observation planète (voir Navigation / Scène 3D).
