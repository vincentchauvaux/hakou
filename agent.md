...
- **Studio visuel / ceintures** (20 sept. 2026) : **un seul univers**. Stream = cadrage §2 Uranus + offsets. Son spectateur = **code du live** (cookie `hakou_listen`) ou session allowlist Vincent/Anaïs. HLS/WHEP à nouveau `auth_request`. Intro : traversée des yeux, logo inversé, disparition. Cache `?v=20260920aj`.
...
- **Revue architecture 20 sept. 2026** : canvas [`revue-securite-hakou.canvas.tsx`](/Users/hakou/.cursor/projects/Users-hakou-hakou/canvases/revue-securite-hakou.canvas.tsx). **Patch élevés** : `scene3d.js` servi depuis l’origine studio (`sync-world.sh`, plus d’import hakou.be / retiré de `script-src`) ; `/index.html` derrière `requireAuthHtml` ; `?preview=1` hors prod seulement ; HLS sans `?cookieCheck=1`. **Stream public** : status ouvert (sans `hlsUrl` sans cookie) ; HLS/WHEP + WHIP + chat + archives allowlist/code. Cache `?v=20260920aj`.
...
- **Stream** : mix privé. Studio allowlist. Spectateurs hakou.be : code affiché à **En direct**. Plexus visibles sans code. HLS cookie `hakou_listen`. Cache `?v=20260920aj`.
...
- **Scène** (`scene3d.js`) : groupe `introGate` — logo SVG vectoriel (stable, **sans bounce**). Au repos : fond `#000` + **univers masqué**. Clic logo → **traversée des yeux** (~4,2 s) : caméra vise le milieu des orbites, passe le plan, **logo inversé** (`rotation.y` π) puis **disparaît**, cadrage §0. Fond **reste noir** ; **pas de nébuleuses**.
