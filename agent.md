...
- **Studio visuel / ceintures** (20 sept. 2026) : un système solaire, 4 spots. Capture mix avant le live (vu-mètre). Pouls `POST /api/studio/pulse` (session) → spectateurs. Cache `?v=20260920y`.
...
- **Revue architecture 20 sept. 2026** : canvas [`revue-securite-hakou.canvas.tsx`](/Users/hakou/.cursor/projects/Users-hakou-hakou/canvases/revue-securite-hakou.canvas.tsx). **Patch élevés** : `scene3d.js` servi depuis l’origine studio (`sync-world.sh`, plus d’import hakou.be / retiré de `script-src`) ; `/index.html` derrière `requireAuthHtml` ; `?preview=1` hors prod seulement ; HLS sans `?cookieCheck=1`. **Stream public** : status/HLS/WHEP ouverts, WHIP + chat + archives toujours allowlist. Cache `?v=20260920y`.
...
- **Dernier redéploiement** : 20 sept. 2026 — studio mix+vu-mètre, HLS iPad, pouls plexus ; cache `?v=20260920y`.
