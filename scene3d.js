...
const FOCUS_ORBIT_ELEV_MAX = 1.4;
const FOCUS_ORBIT_ZOOM_SENS = 0.0017;
const FOCUS_ORBIT_RADIUS_MIN_MUL = 2.15;
const FOCUS_ORBIT_RADIUS_MAX_MUL = 16;
/** Inertie caméra après drag (mode Voir) — décroissance expo → repos. */
const FOCUS_ORBIT_INERTIA_DAMP = 3.6;
const FOCUS_ORBIT_INERTIA_STOP = 0.0012;
...
function getFocusPivot(sectionIndex, out, elapsed = 0, displaySection = sectionIndex, glideState = null) {
  const entry = planetEntries[sectionIndex];
  if (entry?.mesh) {
    out.copy(entry.mesh.position);
    return out;
  }
  const planet = PLANETS[sectionIndex];
  if (planet) {
    return getPlanetPosition(planet, elapsed, displaySection, out, glideState);
  }
  if (sectionCameras[sectionIndex]?.lookAt) {
    out.copy(sectionCameras[sectionIndex].lookAt);
    return out;
  }
  out.set(0, 0, 0);
  return out;
...
function updateFocusOrbitRadiusLimits(sectionIndex) {
  ...
  focusOrbitRadiusMin = Math.max(planet.size * FOCUS_ORBIT_RADIUS_MIN_MUL, 0.9);
  focusOrbitRadiusMax = Math.max(
    planet.size * FOCUS_ORBIT_RADIUS_MAX_MUL,
    focusOrbitRadiusMin * 3
  );
}
...
function clearOrbitVelocity(sectionIndex = lastAtRestSectionIndex) {
  const orbit = sectionUserOrbit[sectionIndex];
  if (!orbit) return;
  orbit.azVel = 0;
  orbit.elVel = 0;
...
function ensureFocusOrbitCaptured(sectionIndex = lastAtRestSectionIndex) {
  ...
  if (!orbit) return;
  if (!orbit.modified) {
    captureOrbitFromCamera(sectionIndex);
  } else {
    updateFocusOrbitRadiusLimits(sectionIndex);
  }
}
...
function pinchDistance() {
  const pts = [...orbitDrag.pointers.values()];
...
function syncOrbitGrabbingClass() {
  if (!orbitCanvas) return;
  const grabbing =
    planetFocusMode &&
    (orbitDrag.active || orbitDrag.pinch || orbitDrag.pointers.size > 0);
  orbitCanvas.classList.toggle("orbit-grabbing", grabbing);
  document.body.style.userSelect = grabbing ? "none" : "";
}
...
function endOrbitDrag() {
  ...
  if (orbitCanvas) {
    for (const id of ids) {
      if (orbitCanvas.hasPointerCapture(id)) {
        orbitCanvas.releasePointerCapture(id);
      }
    }
...
function onOrbitPointerDown(event) {
  if (!canOrbitDrag()) return;
  if (event.pointerType === "mouse" && event.button !== 0) return;

  orbitDrag.pointers.set(event.pointerId, {
    x: event.clientX,
    y: event.clientY,
  });
  try {
...
function onOrbitPointerMove(event) {
  if (!orbitDrag.pointers.has(event.pointerId)) return;
  orbitDrag.pointers.set(event.pointerId, {
    x: event.clientX,
    y: event.clientY,
  });
  ...
  const azV = azDelta / dt;
  const elV = elDelta / dt;
  const s = FOCUS_ORBIT_VEL_SMOOTH;
  orbit.azVel = orbit.azVel * (1 - s) + azV * s;
  orbit.elVel = orbit.elVel * (1 - s) + elV * s;
  event.preventDefault();
}

function onOrbitPointerUp(event) {
  if (!orbitDrag.pointers.has(event.pointerId)) return;
  orbitDrag.pointers.delete(event.pointerId);
...
function tickFocusOrbitInertia() {
  if (!planetFocusMode || orbitDrag.active || orbitDrag.pinch) {
  ...
  const now = performance.now();
  const dt = Math.min(
    Math.max((now - (focusOrbitLastTickMs || now)) / 1000, 0),
    0.05
  );
  focusOrbitLastTickMs = now;
  if (dt <= 0) return;

  const speed = Math.hypot(orbit.azVel, orbit.elVel);
  ...
  orbit.azimuth += orbit.azVel * dt;
  orbit.elevation = clamp(
    orbit.elevation + orbit.elVel * dt,
    FOCUS_ORBIT_ELEV_MIN,
    FOCUS_ORBIT_ELEV_MAX
  );
  if (
    orbit.elevation <= FOCUS_ORBIT_ELEV_MIN + 1e-4 ||
    orbit.elevation >= FOCUS_ORBIT_ELEV_MAX - 1e-4
  ) {
    orbit.elVel = 0;
  }
  const damp = Math.exp(-FOCUS_ORBIT_INERTIA_DAMP * dt);
  orbit.azVel *= damp;
  orbit.elVel *= damp;
}
...
function onOrbitLostPointerCapture(event) {
  if (orbitDrag.pointers.has(event.pointerId)) {
    onOrbitPointerUp(event);
  }
}
...
function initRestOrbitInteraction(canvas) {
  ...
  canvas.addEventListener("lostpointercapture", onOrbitLostPointerCapture);
...
function disposeRestOrbitInteraction() {
  ...
  orbitCanvas.removeEventListener("lostpointercapture", onOrbitLostPointerCapture);
...
function updateCamera(displaySection, elapsed, glideState, settleT = 1) {
  ...
  if (introSnapFrames < INTRO_SNAP_FRAMES) {
    ...
    const driftScale = introAtRest || inGlide || focusOrbit
      ? 0
    ...
    // Collision douce uniquement hors glide / hors observation libre.
    if (!inGlide && !focusOrbit) {
      pushPointOutsideSun(
        camera.position,
        getSunPushExtraMargin(sectionIndex, sectionIndex, displaySection) * 0.5
      );
      enforceMinSunViewDistance(sectionIndex, camera.position);
    } else if (focusOrbit && activePlanet) {
      // Garde une distance mini à la surface pendant le zoom.
      const minDist = activePlanet.size * 1.2;
      tmpSeg.copy(camera.position).sub(tmpFocusPivot);
      const dist = tmpSeg.length();
      if (dist > 1e-5 && dist < minDist) {
        tmpSeg.multiplyScalar(minDist / dist);
        camera.position.copy(tmpFocusPivot).add(tmpSeg);
        smoothedCamPos.copy(camera.position);
        const focusOrbitState = sectionUserOrbit[sectionIndex];
        if (focusOrbitState) focusOrbitState.radius = minDist;
      }
    }
    if (userOrbit) {
      smoothedCamPos.copy(camera.position);
...
function fillStreamSpotCamera(elapsed) {
  ...
  const len = streamHeroDir.length() || 1;
  streamHeroDir.normalize();
  streamTouchRight.crossVectors(tmpUp, streamHeroDir);
...
function applyStreamSpotCamera(elapsed) {
...
export function renderScene(displaySection, glideState = null) {
  ...
  if (
    ...
    resetRestOrbitOffsets(getActiveSectionIndex(displaySection, glideState));
  }
